...
ואז גש ל-[http://localhost:3000/admin/login](http://localhost:3000/admin/login).

מה יש בדשבורד:
- **לוח בקרה (`/admin`)** — סטטיסטיקות (סה״כ הזמנות / שולמו / ממתינות / מקדמות שנגבו / פייפליין) + טבלת 25 הזמנות אחרונות
- **רשימת הזמנות (`/admin/orders`)** — כל ההזמנות במערכת
- **דף הזמנה (`/admin/orders/[id]`)** — פרטי לקוח, פרטי משרד, תמחור מלא, סטטוס תשלום

האימות הוא סיסמה משותפת אחת + קוקי httpOnly חתום HMAC. רוטציה של `ADMIN_PASSWORD`
פוסלת את כל ה-sessions מיידית. כשהדרישות יגדלו אפשר להחליף את `lib/admin-auth.ts`
ב-Supabase auth + טבלת roles.
